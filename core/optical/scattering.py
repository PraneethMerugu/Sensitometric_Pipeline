...
class OpticalPhysics(eqx.Module):
    ...
    black_offset: jax.Array # Correction for LUT non-zero black level
    ...
    def __init__(
        ...
        # CORRECTION: Pre-compute the halation generated by absolute black (0,0,0)
        # The LUT may have a non-zero floor. We subtract this to ensure dark signals don't halate.
        black_rgb = jnp.zeros((1, 1, 3))
        black_energy = self.upsampler.upsample(black_rgb).reshape(-1)
        self.black_offset = jnp.dot(black_energy, self.transmission_spectrum)
    ...
    def compute_halation_mask(self, rgb_image: jax.Array) -> jax.Array:
        ...
        # 3. Apply Black Level Correction
        # Subtract the "background noise" of the LUT
        halation_intensity = jnp.maximum(halation_intensity - self.black_offset, 0.0)
    ...
    def __call__(self, actinic_image: jax.Array) -> jax.Array:
        ...
        # Note: (1.0 - mix) + mix * otf computes the weighted average in freq domain
